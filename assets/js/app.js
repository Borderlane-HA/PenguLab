(() => {
  ...
  function openNpmplusImportModal(integration){
    ...
    showModal('NPMplus Apps importieren','Proxy Hosts werden direkt aus NPMplus gelesen. Du entscheidest, welche Einträge PenguLab übernimmt.',`<div class="npmplus-import-loading"><span class="spinner"></span><strong>Proxy Hosts werden geladen…</strong><small>PenguLab meldet sich nur für diesen Abruf an NPMplus an.</small></div>`,`<button class="btn" data-close-modal>Schließen</button>`,async modal=>{
      const body=$('.modal-body',modal),footer=$('.modal-footer',modal);
      try{
        const d=await api('npmplus/hosts',{body:{id:integration.id}});if(d.integrations)state.boot.integrations=d.integrations;
        const hosts=Array.isArray(d.hosts)?d.hosts:[];const usable=hosts.filter(h=>h.importable);const defaultCategory=integration?.config?.last_import_category||'NPMplus';
        body.innerHTML=`<div class="npmplus-import-tools"><div class="field-row"><label>Kategorie für neue Apps</label><input id="npmImportCategory" value="${attr(defaultCategory)}" placeholder="NPMplus"></div><div class="field-row"><label>Dashboard</label><select id="npmDashboardMode"><option value="library">Nur App-Bibliothek</option><option value="group">Als eine App-Gruppe</option><option value="individual">Als einzelne Kacheln</option></select></div><div class="field-row npm-group-name" id="npmGroupNameRow"><label>Name der App-Gruppe</label><input id="npmGroupName" value="NPMplus"></div><label class="setting-line compact npm-update-existing"><span class="setting-copy"><strong>Bereits importierte Apps aktualisieren</strong><p>PenguLab aktualisiert die Ziel-URL, behält aber eigene Namen, Icons und Kategorien bei.</p></span><input id="npmUpdateExisting" type="checkbox" checked></label></div><div class="npmplus-import-head"><div><strong>${fmt(hosts.length)} Proxy Hosts</strong><span>${fmt(usable.filter(h=>h.enabled).length)} aktiv importierbar</span></div><div><button class="btn small soft" id="npmSelectActive">Alle aktiven</button><button class="btn small" id="npmSelectNone">Keine</button></div></div><div class="npmplus-host-list">${hosts.length?hosts.map(h=>{const unavailable=!h.importable;const checked=h.enabled&&h.importable&&!h.imported;const stateLabel=h.imported?`Bereits importiert${h.app_name?` · ${h.app_name}`:''}`:(h.existing?'Bestehende App gefunden':(h.enabled?'Aktiv':'Deaktiviert'));return `<label class="npmplus-host-row ${unavailable?'disabled':''}"><input type="checkbox" data-npm-host="${attr(h.key)}" ${checked?'checked':''} ${unavailable?'disabled':''}><span class="npmplus-host-main"><strong>${esc(h.domain||'Unbekannter Host')}</strong><small>${esc(h.backend||'Kein Backend gemeldet')}</small></span><span class="npmplus-host-badges">${h.https?'<i class="npm-badge https">HTTPS</i>':'<i class="npm-badge">HTTP</i>'}${!h.enabled?'<i class="npm-badge disabled">Aus</i>':''}${h.imported?'<i class="npm-badge imported">Importiert</i>':(h.existing?'<i class="npm-badge existing">Vorhanden</i>':'')}</span><span class="npmplus-host-state">${esc(unavailable?'Wildcard nicht direkt importierbar':stateLabel)}</span></label>`}).join(''):'<div class="widget-loading">NPMplus hat keine Proxy Hosts zurückgegeben.</div>'}</div>`;
        footer.innerHTML=`<button class="btn" data-close-modal>Abbrechen</button><button class="btn primary" id="npmImportSelected" ${usable.length?'':'disabled'}>Ausgewählte importieren</button>`;$$('[data-close-modal]',footer).forEach(b=>b.addEventListener('click',closeModal));
        const mode=$('#npmDashboardMode',modal),groupRow=$('#npmGroupNameRow',modal);const syncMode=()=>groupRow?.classList.toggle('visible',mode?.value==='group');mode?.addEventListener('change',syncMode);syncMode();
        $('#npmSelectActive',modal)?.addEventListener('click',()=>$$('[data-npm-host]',modal).forEach(el=>{if(!el.disabled)el.checked=hosts.find(h=>h.key===el.dataset.npmHost)?.enabled!==false;}));
        $('#npmSelectNone',modal)?.addEventListener('click',()=>$$('[data-npm-host]',modal).forEach(el=>el.checked=false));
        $('#npmImportSelected',modal)?.addEventListener('click',async()=>{const keys=$$('[data-npm-host]',modal).filter(el=>el.checked&&!el.disabled).map(el=>el.dataset.npmHost);if(!keys.length){toast('Wähle mindestens einen Proxy Host.','error');return;}const btn=$('#npmImportSelected',modal);btn.disabled=true;btn.textContent='Importiere…';try{const result=await api('npmplus/import',{body:{id:integration.id,keys,category:$('#npmImportCategory',modal).value,dashboard_mode:mode.value,group_name:$('#npmGroupName',modal)?.value||'NPMplus',update_existing:$('#npmUpdateExisting',modal).checked}});state.boot.apps=result.apps||state.boot.apps;state.boot.widgets=result.widgets||state.boot.widgets;state.boot.integrations=result.integrations||state.boot.integrations;closeModal();renderIntegrations();const parts=[];if(result.created)parts.push(`${result.created} neu`);if(result.updated)parts.push(`${result.updated} aktualisiert`);if(result.reused)parts.push(`${result.reused} vorhandene übernommen`);toast(`NPMplus Import fertig${parts.length?`: ${parts.join(' · ')}`:''}.`);}catch(e){btn.disabled=false;btn.textContent='Ausgewählte importieren';toast(e.message,'error')}});
      }catch(e){body.innerHTML=`<div class="npmplus-import-error"><strong>Proxy Hosts konnten nicht geladen werden.</strong><p>${esc(e.message)}</p><small>Prüfe NPMplus URL, Login, Passwort und ggf. die TLS-Einstellung.</small></div>`;toast(e.message,'error');}
    });
  }
